              ...
              <div className="col-span-2">
                <div className="flex flex-col h-full">
                  <div className="h-full flex flex-col justify-between">
                    <div className="mx-4 align-top flex flex-col gap-y-2 ">
                      <Header1>Shop By Occasion</Header1>
                      <Header2>
                        Styles for special events & everyday moments
                      </Header2>
                    </div>
                    <div className="flex mx-4 items-center w-2/3 justify-between mb-2">
                      {/* <div className="flex gap-6 justify-between">
                        {occassion.map((city, id) => (
                          <div key={id}>
                            <button
                            ...
                            >
                              <span className="block mt-2 text-center text-xl font-bold">
                                {city.name}
                              </span>
                              <div
                                className={`duration-500 w-0 group-hover/category:w-full rounded-full border-b-2 border-b-black`}
                      ...
                      <div className="flex  justify-between">
                        {occassion.map((city, id) => (
                          <div key={id} className="flex items-center">
                            <button
                              className={`flex flex-col items-center justify-center group/category  duration-500 ${
                                isActive ? "" : ""
                              }`}
                              onClick={() => filterItem(city.name, city.id)}
                            >
                              <span className="block mt-2 text-center text-xl font-bold group-hover/category:text-[#dcb06a] duration-300">
                                {city.name}
                              </span>
                              <div
                                className={`duration-500 w-0 group-hover/category:w-full rounded-full border-b-[1px] border-b-[#dcb06a] `}
                              ></div>
                            </button>

                            {/* Add vertical line except after the last item */}
                            {id !== occassion.length - 1 && (
                              <div className="h-4 border-l-[2px] border-black mx-4"></div>
                            )}
                          </div>
                        ))}
                      </div>
                        ...
                        {propertyList.propertyOnPage.map((property, index) => (
                          <div key={index} className="">
                            <Link href={`/product/${property.id}`}>
                              ...
                              <div className="duration-500 text-xl line-clamp-2 px-4 mt-2 ">
                                {property.name}
                              </div>
                            </Link>
                          </div>
                  ...
                  <div className="flex justify-between items-center w-full">
                    ...
                    {selectedOccasion && (
                      <div className="">
                        <button
                          onClick={handleViewMore}
                          className="py-2 px-4 bg-[#533265] text-white font-bold rounded-full hover:bg-[#533265] transition duration-300"
                        >
                          View More
                        </button>
                      </div>
