...
const ShopByPattern = ({ Patterns }) => {
  const [activePatternIndex, setActivePatternIndex] = useState(0); // To track the active pattern

  // Handler to update the active slide index
  const handleSlideChange = (swiper) => {
    setActivePatternIndex(swiper.realIndex);
  };
            ...
            {Patterns.map((pattern, index) => (
              <SwiperSlide key={index} className="swiper-slide ">
                <Link href={`/products?patternId=${pattern.id}`} className="">
                  ...
                  {/* <p className="text-center mt-2 text-sm md:text-lg font-semibold">
                    {pattern.name}
                  </p> */}
                </Link>
              </SwiperSlide>
