...
export default function ShopByCategory({ types }) {
  ...
  const uniqueTypes = types.reduce((acc, curr) => {
    // Check if the current type's name already exists in the accumulator
    if (!acc.some((item) => item.name === curr.name)) {
      acc.push({
        name: curr.name,
        imageUrl: curr.imageUrl, // Add any other necessary fields
        id: curr.id,
      });
    }
    return acc;
      ...
      {uniqueTypes.length >= 1 && (
        <div className="">
          <div className="container mx-auto flex flex-col gap-y-2 px-4 sm:px-8">
            ...
            <div className="relative section-swiper-navigation  grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-6 ">
              {uniqueTypes.map((slide, index) => (
                <Link href={`/products?typeId=${slide.id}`} key={index}>
                  ...
                  <div className="group hover:scale-105 duration-300 transform transition ">
                    ...
                    <div className="text-base  md:text-2xl transition duration-700  group-hover:text-lg md:group-hover:text-2xl">
                      {slide.name}
                    </div>
                  </div>
                </Link>
