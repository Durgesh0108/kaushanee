...
export default function ProductInsightPage({
  ...
  useEffect(() => {
    const checkCartStatus = async () => {
      if (!userId) {
        return; // If user is not logged in, do not check the cart status
      }
      try {
  ...
  const handleCartAction = async () => {
    ...
    try {
      if (isInCart) {
        // Remove from cart
        await axios.delete(`/api/cart/${userId}/product/${product.id}`);
      ...
      toast.error("Error performing cart action");
  ...
  return (
    <>
      <div className="container mx-auto mt-8">
        {/* Product Images */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 bg-white rounded-xl border-[1px] border-[#533265]/80 mb-8">
          <div className="p-4 md:p-6">
            <Tab.Group>
              <div className="flex flex-col md:flex-row">
                <Tab.List className="flex md:flex-col gap-4 md:gap-2 overflow-x-auto no-scrollbar md:overflow-y-scroll md:h-full overflow-auto md:w-1/4">
                ...
                <Tab.Panels className="flex-1 h-full relative">
                  {product?.images?.map((image, index) => (
                    <Tab.Panel
                      key={index}
                      className="h-full w-full flex items-center justify-center bg-gray-100 p-4 cursor-pointer"
                      onClick={openModal}
                    >
          ...
          <div className="flex flex-col gap-8 p-4">
            ...
            <div className="flex gap-4 mt-4">
              ...
              className="flex items-center gap-2 py-2 px-4 bg-[#533265] text-white font-bold rounded-lg"
            >
        ...
        <div className="bg-white rounded-xl border-[1px] border-[#533265]/80 mb-8  no-scrollbar overflow-auto px-4 py-12 flex flex-col gap-4">
          <div>
        ...
        <div className="bg-white rounded-xl border-[1px] border-[#533265]/80 mb-8 p-4">
      ...
      {isModalOpen && (
        <ImageModal showModal={isModalOpen} closeModal={closeModal}>
          <Swiper
            slidesPerView={1}
            spaceBetween={10}
            loop={true}
